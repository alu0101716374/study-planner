...
def render_availability_ui():
    ...
    if "current_availability" not in st.session_state or st.session_state.reload_availability:
        st.session_state.current_availability = repository.get_user_availability(user_id=user_id)
    ...
    for day in days:
        ...
        default_pref_str = loaded_availability.get(day, {}).get("preference", AVAILABILITY_PREF_OPTIONS[0]).capitalize()
        
        default_pref_idx = AVAILABILITY_PREF_OPTIONS.index(default_pref_str) if default_pref_str in AVAILABILITY_PREF_OPTIONS else 0

        col1, col2 = st.columns(2)
        with col1:
           
            h = st.number_input(f"Hours: {day}", 0.0, 24.0, value=default_hours, step=0.5, key=f"h_{day}")
        with col2:
            p = st.selectbox(f"Pref: {day}", AVAILABILITY_PREF_OPTIONS, index=default_pref_idx, key=f"p_{day}")
    ...
    if st.button("Save Changes"):
        repository.update_user_availability(user_id=user_id, availability_data=weekly_data)
    ...
    with b_col1:
        blanket_hours = st.number_input("Hours for ALL days", 0.0, 24.0, step=0.5, key="blanket_hours")
    with b_col2:
        blanket_pref = st.selectbox("Preference for ALL days", AVAILABILITY_PREF_OPTIONS, key="blanket_pref")
    ...
    if st.button("Apply to All Days & Save"):
        ...
        repository.update_user_availability(user_id=user_id, availability_data=updated_weekly_data_for_blanket)
